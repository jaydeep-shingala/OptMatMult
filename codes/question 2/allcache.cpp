...
#include "pin_cache.H"
std::ofstream TraceFile("benchmark_3_trace.out");
uint64_t start_time;
...
static VOID Ul2Access(ADDRINT addr, UINT32 size, CACHE_BASE::ACCESS_TYPE accessType)
{
    ...
    if (!ul2Hit)
    {
  	...
  	// stream << "0x" << std::setfill ('0') << std::setw(8) << std::hex << std::setw(8) << (addr&0x3ffffffff);
 
    	if(accessType)
    	{
    		TraceFile << std::hex << (void *)(addr&(0x0003ffffffff)) << " " << "WRITE" << " " << std::dec << (end_time - start_time)/262 << std::endl;	
    	}
    	else
    	{
    		TraceFile << std::hex << (void *)(addr&(0x0003ffffffff)) << " " << "READ" << "  " << std::dec << (end_time - start_time)/262 << std::endl;
    	}
    }
}
